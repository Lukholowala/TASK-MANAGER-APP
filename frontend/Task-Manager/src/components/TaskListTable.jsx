...
const TaskListTable = ({tableData}) => {
    ...
    const getPriorityBadgeColor = (priority) => {
        switch (priority) {
            ...
            case 'Low': return 'bg-green-100 text-green-500 border border-gray-200';
            default: return 'bg-gray-100 text-gray-500 border borderpgray-200';
        }
    };
    return (
                ...
                <tbody>
                    {tableData.map((task)=>(
                        <tr key={task.id} className="border-t border-gray-200">
                            ...
                            <td className="py-4 px-4">
                                <span className={`px-2 py-1 text-xs rounded inline-block ${getPriorityBadgeColor(task.priority)}`}>{task.priority}</span></td>
